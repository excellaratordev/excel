...
from superexcel.core.permissions import role_allows
...
def workbook_access(workbook_id: int):
    ...
    if not response.ok:
        return None, None, api_error(response, "Erro ao localizar planilha")
    if not workbook:
    ...
    if not role_allows(role, "telemetry.view"):
        return None, None, (jsonify({"error": "Você não possui acesso às métricas."}), 403)
...
def record_workbook_telemetry(workbook_id: int):
    workbook, role, error = workbook_access(workbook_id)
    if error:
        return error
    ...
    return jsonify({"workbook_id": workbook_id, "recorded": recorded})


@telemetry_api.get("/api/workbooks/<int:workbook_id>/telemetry")
def workbook_telemetry(workbook_id: int):
    workbook, _, error = workbook_access(workbook_id)
    if error:
        return error
    ...
    return jsonify({
        "workbook": {
            ...
            "revision": workbook.get("revision"),
        },
...
def project_telemetry(project_id: int):
    ...
    if error:
        return error
    if not role_allows(role, "telemetry.view"):
        return jsonify({"error": "Você não possui acesso às métricas."}), 403
    ...
    return jsonify({
        ...
        "workbooks": [
            {**item, "runtime": summaries[int(item["id"])]}
