...
def list_workbooks():
    ...
    response = db("GET", "workbooks", params={
        "select": "id,name,folder_id,project_id,revision,created_at,updated_at,updated_by_email",
...
def save_workbook():
    ...
    if workbook_id is None:
        ...
        return jsonify({"id": saved["id"], "name": saved["name"], "revision": saved.get("revision", 1), "updated_at": saved["updated_at"], "updated_by_email": email})
    ...
    saved = response.json()[0]
    return jsonify({"id": saved["id"], "name": saved["name"], "revision": saved["revision"], "updated_at": saved["updated_at"], "updated_by_email": email})
