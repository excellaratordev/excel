(() => {
  'use strict';
  const ROWS = 60, COLS = 26, AUTOSAVE = 'super-excel-autosave-v1';
  const F = [
    ['SOMA','Soma valores','=SOMA(B2:B20)'],['MÉDIA','Calcula a média','=MÉDIA(B2:B20)'],['MÁXIMO','Maior valor','=MÁXIMO(B2:B20)'],['MÍNIMO','Menor valor','=MÍNIMO(B2:B20)'],
    ['SE','Executa uma condição','=SE(B2>=1000;"Meta atingida";"Abaixo da meta")'],['SES','Várias condições','=SES(B2>=1000;"Alta";B2>=500;"Média";VERDADEIRO;"Baixa")'],
    ['E','Todas as condições','=E(B2>0;C2="Pago")'],['OU','Qualquer condição','=OU(C2="Pago";C2="Parcial")'],['SEERRO','Substitui erros','=SEERRO(A2/B2;0)'],
    ['CONT.NÚM','Conta números','=CONT.NÚM(B2:B100)'],['CONT.VALORES','Conta células preenchidas','=CONT.VALORES(A2:A100)'],['CONT.SE','Conta por critério','=CONT.SE(C2:C100;"Pago")'],
    ['CONT.SES','Conta por critérios','=CONT.SES(C2:C100;"Pago";B2:B100;">1000")'],['SOMASE','Soma por critério','=SOMASE(C2:C100;"Pago";B2:B100)'],
    ['SOMASES','Soma por critérios','=SOMASES(D2:D100;B2:B100;"Fortaleza";C2:C100;"Pago")'],['MÉDIASE','Média por critério','=MÉDIASE(C2:C100;"William";B2:B100)'],
    ['MÉDIASES','Média por critérios','=MÉDIASES(D2:D100;B2:B100;"Fortaleza";C2:C100;"Pago")'],['PROCV','Busca vertical','=PROCV(A2;F2:H20;3;FALSO)'],
    ['PROCX','Busca moderna','=PROCX(A2;F2:F20;H2:H20;"Não encontrado")'],['ÍNDICE','Valor pela posição','=ÍNDICE(D2:D100;5)'],['CORRESP','Posição de um valor','=CORRESP(A2;F2:F100;0)'],
    ['FILTRO','Filtra linhas','=FILTRO(A2:D20;D2:D20="Pendente")'],['ÚNICO','Valores únicos','=ÚNICO(B2:B20)'],['CLASSIFICAR','Ordena intervalo','=CLASSIFICAR(A2:D20;4;-1)'],
    ['CONCAT','Junta textos','=CONCAT(A2;" - ";B2)'],['TEXTO.JUNTAR','Junta com separador','=TEXTO.JUNTAR(", ";VERDADEIRO;A2:A10)'],
    ['ESQUERDA','Texto do início','=ESQUERDA(A2;3)'],['DIREITA','Texto do final','=DIREITA(A2;4)'],['TEXTO','Formata valor','=TEXTO(B2;"R$ #.##0,00")'],['HOJE','Data atual','=HOJE()']
  ];
  ...
  function cell(r,c){ return el.grid.querySelector(`[data-row="${r}"][data-col="${c}"]`); }
  function status(text,error=false){ el.status.textContent=text; el.status.classList.toggle('error',error); }
  function parse(v){ const t=String(v).trim(); if(!t)return null; if(t.startsWith('='))return t; if(/^-?(?:\d{1,3}(?:\.\d{3})+|\d+)(?:,\d+)?$/u.test(t))return Number(t.replaceAll('.','').replace(',','.')); if(/^(VERDADEIRO|FALSO)$/iu.test(t))return t.toUpperCase()==='VERDADEIRO'; return v; }
  function raw(r,c){ return data[r]?.[c] ?? null; }
  ...
  async function openServer(id){ const res=await fetch(`/api/workbooks/${id}`), out=await res.json(); if(!res.ok)throw new Error(out.error||'Erro ao abrir.'); load(out.data,out.id); el.oDlg.close(); status(`Planilha aberta: ${out.name}`); }
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-close-dialog]').forEach(b=>b.onclick=()=>document.querySelector(`#${b.dataset.closeDialog}`).close()); el.name.addEventListener('input',autosave); window.addEventListener('beforeunload',()=>localStorage.setItem(AUTOSAVE,JSON.stringify(serialize())));
  }
  function init(){ build();renderFunctions();rebuild();bind();const saved=localStorage.getItem(AUTOSAVE);if(saved){try{load(JSON.parse(saved));status('Rascunho restaurado do navegador');}catch{localStorage.removeItem(AUTOSAVE);select(0,0);}}else select(0,0); }
  init();
})();