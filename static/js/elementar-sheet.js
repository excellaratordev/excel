(() => {
  ...
  function currentEndpoint() {
    ...
    return absoluteUrl(state.config.visibility === 'public' ? state.config.public_endpoint : state.config.authenticated_endpoint);
  }
  ...
  function renderConfig() {
    ...
    if (version) version.textContent = state.config.last_publication_version ? `v${state.config.last_publication_version}` : 'Não publicada';
    const latest = state.config.publication;
    setStatus(latest?.created_at
      ? `Última publicação em ${new Date(latest.created_at).toLocaleString('pt-BR')}.`
      : 'Declare os elementos, gere uma prévia e publique o JSON.');
  }

  function colIndex(name) {
    let result = 0;
    for (const letter of String(name).replaceAll('$', '').toUpperCase()) result = result * 26 + letter.charCodeAt(0) - 64;
    return result - 1;
  }
  ...
  function cellAddress(row, col) {
    let name = '';
    for (let number = col + 1; number; number = Math.floor((number - 1) / 26)) name = String.fromCharCode(65 + ((number - 1) % 26)) + name;
    return `${name}${row + 1}`;
  }
  ...
  function payloadEntries(snapshot) {
    ...
    if (sparse) {
      return cells.map(item => ({ row: Number(item?.r), col: Number(item?.c), value: item?.v ?? item?.value ?? null }))
        .filter(item => Number.isInteger(item.row) && Number.isInteger(item.col) && item.row >= 0 && item.col >= 0);
    }
    const entries = [];
    cells.forEach((row, rowIndex) => {
      if (!Array.isArray(row)) return;
      row.forEach((value, col) => { if (!isBlank(value)) entries.push({ row: rowIndex, col, value }); });
    });
    return entries;
  }
  ...
  function payloadMatrix(payload) {
    ...
    let lastRow = -1;
    let lastCol = -1;
    entries.forEach(item => { lastRow = Math.max(lastRow, item.row); lastCol = Math.max(lastCol, item.col); });
  ...
  function parseDeclarations(snapshot) {
    ...
    payloadEntries(snapshot).forEach(({ row, col, value }) => {
      ...
      seen.add(key);
      declarations.push({ key, workbook_name: match[2].replaceAll("''", "'"), range: match[3].replaceAll('$', '').toUpperCase(), cell: cellAddress(row, col) });
    });
    if (invalid.length) throw new Error(`Declaração inválida em ${invalid.slice(0, 6).join(', ')}. Use nome='Planilha'!A1:D100.`);
    if (!declarations.length) throw new Error("Nenhuma declaração encontrada. Exemplo: pedidos='Planilha de Pedidos'!A1:D100");
    return declarations;
  }
  ...
  function jsonValue(engine, row, col, sourceName) {
    ...
    if (typeof value === 'string' && value.startsWith('#')) throw new Error(`${sourceName}!${cellAddress(row, col)} contém o erro ${value}.`);
  ...
  function matrixToJson(matrix) {
    ...
    if (validHeaders) {
      return matrix.slice(1).filter(row => row.some(value => !isBlank(value)))
  ...
  function setNested(target, path, value) {
    ...
    parts.forEach((part, index) => {
      ...
      if (!current[part] || typeof current[part] !== 'object' || Array.isArray(current[part])) throw new Error(`O caminho ${path} entra em conflito com outro elemento.`);
  ...
  async function buildPreview() {
    ...
    const resolved = await api(`/api/elementar/workbooks/${workbookId}/resolve`, { method: 'POST', body: JSON.stringify({ references: declarations }) });
    ...
    try {
      for (const declaration of resolved.references) {
        ...
        for (let row = bounds.top; row <= bounds.bottom; row += 1) {
          const values = [];
          for (let col = bounds.left; col <= bounds.right; col += 1) values.push(jsonValue(engine, row, col, source.name));
          matrix.push(values);
        }
  ...
  async function publish() {
    ...
    try {
      const output = await api(`/api/elementar/workbooks/${workbookId}/publish`, {
        method: 'POST',
        body: JSON.stringify({ payload: preview.payload, declarations: preview.declarations, definition_revision: preview.definitionRevision, sources: preview.sources }),
      });
  ...
  $('#elementar-copy-endpoint')?.addEventListener('click', async () => {
    ...
    if (!value) return;
    try { await navigator.clipboard.writeText(value); setStatus('Endpoint copiado.'); }
    catch { window.prompt('Copie o endpoint:', value); }
  });
  ...
  settingsDialog?.querySelector('form')?.addEventListener('submit', async event => {
    ...
    try {
      const output = await api(`/api/elementar/workbooks/${workbookId}/settings`, {
        method: 'PATCH', body: JSON.stringify({ slug: slugInput.value, visibility: visibilitySelect.value }),
      });
      state.config = { enabled: true, role: state.role, ...output };
      renderConfig();
      settingsDialog.close();
      setStatus('Configurações salvas.');
    } catch (error) { $('#elementar-settings-error').textContent = error.message; }
  });
