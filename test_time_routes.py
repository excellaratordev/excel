...
def stage_view(workbook: dict[str, Any]) -> dict[str, Any]:
    number, label, mode = STAGES.get(str(workbook.get("pipeline_stage") or ""), (0, "Arquivo", "sheet"))
    return {
        "id": int(workbook["id"]),
        "name": workbook["name"],
...
def fetch_session(session_id: str):
    return fetch_one("test_time_sessions", {"select": "*", "id": f"eq.{session_id}"})


def fetch_group(group_id: int):
    return fetch_one("test_time_groups", {"select": "*", "id": f"eq.{group_id}"})


def latest_session(project_id: int, *, open_only: bool = False):
    params = {
        "select": "*",
    ...
    if open_only:
        params["status"] = "in.(setup,running)"
    return fetch_one("test_time_sessions", params)
...
def load_session_state(session: dict[str, Any] | None):
    if not session:
        return [], [], None
    ...
    if not events.ok:
        return [], [], events
    return groups.json(), events.json(), None


@test_time_api.get("/api/test-time/workbooks/<int:workbook_id>")
def state(workbook_id: int):
...
def create_session(workbook_id: int):
    ...
    if error:
        return error
    project_id = int(workbook["project_id"])
    existing, response = latest_session(project_id, open_only=True)
    if not response.ok:
        return api_error(response, "Erro ao procurar uma sessão Test Time")
    if existing:
        return jsonify({"session": existing, "role": role, "existing": True})

    name = str(json_body().get("name") or "Teste de propagação").strip()[:120] or "Teste de propagação"
    created = db(
        "POST",
        "test_time_sessions",
        payload={
    ...
    if created.status_code == 409:
        existing, retry = latest_session(project_id, open_only=True)
...
def start_session(session_id: str):
    try:
        ...
        client_epoch_ms = float(json_body().get("client_epoch_ms"))
    except ValueError as validation_error:
        return jsonify({"error": str(validation_error)}), 400
    session, response = fetch_session(session_id)
    if not response.ok:
    ...
    result = db(
        ...
        payload={"p_session_id": session_id, "p_client_epoch_ms": client_epoch_ms, "p_actor": current_email()},
    )
    if not result.ok:
...
def create_group(session_id: str):
    ...
    try:
        ...
        return jsonify({"error": str(validation_error)}), 400
    workbook, _, workbook_error = require_workbook(workbook_id)
    if workbook_error:
    ...
    group_name = str(body.get("group_name") or f"Grupo {bounds['reference']}").strip()[:120] or f"Grupo {bounds['reference']}"
    number, _, mode = STAGES[str(workbook["pipeline_stage"])]
    created = db(
        "POST",
        "test_time_groups",
        payload={
...
def delete_group(group_id: int):
    group, response = fetch_group(group_id)
    if not response.ok:
    ...
    if error:
        return error
    deleted = db("DELETE", "test_time_groups", params={"id": f"eq.{group_id}"}, prefer="return=minimal")
    if not deleted.ok:
...
def observe_group(group_id: int):
    ...
    changed_cells = body.get("changed_cells") or []
    ...
    try:
        ...
        validate_json_size(body.get("value_preview"), MAX_PREVIEW_BYTES, "A prévia do Test Time excede 32 KB.")
    ...
    observed = db(
        ...
        payload={
            ...
            "p_value_preview": body.get("value_preview"),
...
def base_snapshot(group_id: int):
    group, response = fetch_group(group_id)
    if not response.ok:
        return api_error(response, "Erro ao localizar o grupo Test Time")
    if not group:
    ...
    workbook, role, error = require_workbook(int(group["workbook_id"]))
    if error:
        return error
    ...
    for row_offset in range(row_limit):
        ...
        for column_offset in range(right - left + 1):
            ...
            cells.append({
                ...
                "v": values.get(column["column_key"]) if column else None,
            })
    return jsonify({
        "group_id": group_id,
