...
A prévia é entregue com um sandbox de origem opaca: ela renderiza e executa scripts, mas não consegue ler a sessão, o armazenamento local ou as APIs autenticadas do painel principal.
...
Os HTMLs públicos são servidos em origem separada do painel principal.
...
A rota de prévia adiciona `Content-Security-Policy: sandbox` sem `allow-same-origin` e usa `Cache-Control: no-store`.
